...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            f"pypdb2pov {__version__} -- PDB and mmCIF to POV-Ray {POV_VERSION} scene conversion."
        ),
        epilog=_EPILOG,
    ...
    parser.add_argument(
        ...
        nargs="?",
        metavar="OutputFile",
        help="output scene; '.pov' or '.inc' is appended, and '-' writes to standard output",
    )

    rep = parser.add_argument_group("representation")
    rep.add_argument(
        "-v", "-V", dest="vdw", action="store_true", help="van der Waals radii (default)"
    )
    rep.add_argument("-c", "-C", dest="covalent", action="store_true", help="covalent radii")
    rep.add_argument("-b", "-B", dest="ball_stick", action="store_true", help="ball and stick")
    rep.add_argument(
        "-q", "-Q", dest="glass", action="store_true", help="ball and stick with glass atoms"
    )
    rep.add_argument(
        "-d",
        "-D",
    ...
    scene.add_argument(
        "-p", "-P", dest="plain_sky", action="store_true", help="plain white sky, no ground"
    )
    ...
    inp.add_argument(
        "-t", "-T", dest="atm", action="store_true", help="input is in .atm format rather than PDB"
    )
    inp.add_argument(
        "--format",
        dest="fmt",
    ...
    inp.add_argument(
        "--all-models", dest="all_models", action="store_true", help="convert every model at once"
    )
    inp.add_argument(
        "--altloc",
        dest="altloc",
        choices=[p.value for p in AltLocPolicy],
        default=None,
        help="alternate conformation policy: a (blank or A, the default), first, occupancy, all",
    )
    inp.add_argument(
        "--keep-altlocs",
        dest="keep_altlocs",
    ...
    inp.add_argument(
        "--no-hetatm", dest="no_hetatm", action="store_true", help="skip HETATM records"
    )
    inp.add_argument("--no-water", dest="no_water", action="store_true", help="skip waters")
    inp.add_argument(
        "--legacy-elements",
        dest="legacy_elements",
        action="store_true",
    ...
    inp.add_argument(
        ...
        dest="strict",
        action="store_true",
        help="fail on an unparseable coordinate record instead of skipping it",
    )
    ...
    bond.add_argument(
        "--bonds",
        dest="bond_mode",
        choices=[m.value for m in BondMode],
    ...
    bond.add_argument(
        ...
        default=0.45,
        metavar="X.X",
        help="slack added to the radius sum in covalent mode (default 0.45)",
    )
    ...
    misc.add_argument(
        ...
        dest="name",
        metavar="IDENT",
        help="POV-Ray identifier for the declared objects (default: derived from OutputFile)",
    )
    misc.add_argument(
        "--no-timestamp",
        dest="no_timestamp",
    ...
    misc.add_argument("--quiet", dest="quiet", action="store_true", help="only report problems")
    misc.add_argument(
        "--version",
        action="version",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.list_elements:
        ...
        print(f"{len(ELEMENTS)} elements with a dedicated texture; anything else renders as Atom_X")
        return EXIT_OK
    ...
    bonds = None
    if sopt.ball_stick:
        bonds = find_bonds(structure, sopt.bond_threshold, sopt.bond_mode, sopt.bond_tolerance)
...
def _no_atoms_message(stats, resolved: str, popt: ParseOptions) -> str:
    ...
    if stats.skipped_model:
        return (
            f"pypdb2pov: no atoms in model {popt.model}; the file has {stats.models_seen or 'none'}"
        )
