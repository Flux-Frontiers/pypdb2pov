...
def _malformed(st: ParseStats, opt: ParseOptions, lineno: int, line: str, why: str) -> None:
    if opt.strict:
        raise ParseError(f"line {lineno}: {why}: {line[:72]!r}")
    st.note_malformed(lineno, line)


def apply_altloc_policy(structure: Structure, opt: ParseOptions, st: ParseStats) -> None:
    """
...
def read_structure(path: str, options: ParseOptions | None = None) -> tuple[Structure, ParseStats]:
    """
    ...
    """
    from .mmcif import read_mmcif  # noqa: PLC0415 -- keeps the module optional
